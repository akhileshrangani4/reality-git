    ...
    public static func indices(depth: [Float], confidence: [UInt8], width: Int, height: Int, rect: CGRect) -> [Int] {
        ...
        for y in top...bottom {
            for x in left...right {
                ...
                if u < 0.12 || u > 0.88 || v < 0.12 || v > 0.88 { ring.append(depth[i]) }
        ...
        // At least 75% of the border must be behind the seed, preventing a
        ...
        guard queue.count >= 12, queue.count * 4 < (right-left) * (bottom-top) * 3 else { return [] }
