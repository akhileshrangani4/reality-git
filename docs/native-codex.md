...
Object identity has a separate lifetime from pixel tracking. The first Astra-confirmed RGB/depth snapshot and name remain immutable until an explicit new scan, model change or sign-out. A lost local track, offscreen reply, camera interruption or reconnect to the same model does not replace that memory. The first saved surface is extracted directly when Astra confirms it, even if the camera is paused; a restarted local worker can restore it from that original source. An old saved reference can never supply current-position evidence. Subsequent images keep asking for the same individual object, and its return is `tracked`, not a new `identityConfirmed` selection.

The screen outline is projected from fresh measured world points using each current camera pose, so normal camera movement does not trip the old 1.5 cm image-overlay motion cutoff. That strict cutoff still protects the 2D-only fallback when depth is unavailable. A partially visible pixel track retains its original observation; only the displayed region is clipped. Offscreen or stale geometry is hidden, while the UI says which remembered object it is looking for.
...
`--object-continuity-check` uses the production iOS coordinators and the phone's native Astra connection to select a synthetic object while the camera is paused, refresh after an injected connection loss, submit an empty view, and return from a translated camera. It then checks local tracking/world-position stability, recovery of the original reference after a long gap, and explicit reset. The bounded `Documents/object-continuity-check.json` report contains results only. Camera motion and occlusion are fixture inputs, not a physical walk-around test.
