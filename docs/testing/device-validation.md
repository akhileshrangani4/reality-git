...
## Remember the object through camera movement and return — September 9, 2026

- Removed scan reset on connection availability changes and same-model reconnect. The first Astra-confirmed source frame, label and object identity are retained independently of live tracking. Original surface extraction starts at confirmation, even if the camera is paused. An interrupted worker restores that original source instead of making the next visible frame a replacement reference.
- Reproject fresh measured world surfaces into the current camera view instead of hiding their screen outline when motion since the last pixel observation exceeds 1.5 cm / 0.025 radians. The old strict motion gate remains for the 2D-only fallback. Partial visibility keeps the Vision observation, clipping only the displayed/preview region. Astra still owns full reacquisition after loss.
- Camera interruption, offscreen answers and reconnect preserve the saved reference. Subsequent labels do not rename the selected object. The UI says which remembered object it is looking for, and camera recovery does not restart capture particles. Start over, model change and sign-out still clear the object; Start over now clears it while the camera is paused too.
- The physical iPhone ran `--object-continuity-check` with production coordinators and real native Astra. Initial reference saved while the camera was paused; reconnect preserved the same identity; the empty synthetic view returned `notFound`; return from a translated camera returned `tracked`, not `identityConfirmed`. The original reference key/image/label survived. All 30 simulated camera-translation samples tracked, maximum world-position variation 0.01387 m, with no object-moved state. A restarted worker at a 120-second source age recovered the original reference but produced no stale current position. Explicit reset cleared identity. [Bounded report](object-continuity-device-results.json).
- Live request times were 4.37 s for selection, 2.83 s for the empty view and 8.90 s for return. This fixes scan continuity, not cloud inference latency. Full reacquisition can still wait for Astra. These camera poses and depth frames are fixtures, not a physical walk-around or occlusion/lookalike accuracy claim.
- 87 XCTest + 2 Swift Testing core checks pass, including translated-camera screen projection, leaving/returning to the viewport, partial visibility and same-reference reuse after offscreen/network failure/long gaps. The speed regression check also passes: first preview submitted in 27.94 ms, 119/119 tracks at the 30 Hz fixture cadence, P95 29.54 ms in this run. These timings vary between runs and are not camera FPS guarantees. Signed Debug and Release builds pass. The final signed Release build is installed and launched on the iPhone with its normal camera entry point.

## Fast capture and tracing — September 9, 2026

- Added a provisional red depth surface immediately after tap/box selection, while Astra owns object confirmation and the immutable reference. Provisional geometry never becomes movement evidence. Astra uncertainty, tracking loss, stale geometry and selection changes clear it.
- Cached continuous pixel boxes let a new Astra outline attach to the existing healthy track when its exact source frame and overlapping box match. Gaps and low confidence force reacquisition. New model confirmation grants a bounded ten-second tracking lease; ordinary local frames cannot extend it. The saved source surface is published before recovery replay.
- Camera derivatives, repeated source JPEGs, immutable reference crops/data URLs and HTTP sessions are reused. The renderer preallocates red/green resources, keeps inactive slots transparent, and shortens confirmed capture reveal from 420 to 120 ms. Green remains at the prior 0.045 opacity / 3 mm radius. White particle styling is unchanged.
- The same physical iPhone 15 Pro ran the old and new Debug pipelines against deterministic 1920×1440 camera / 256×192 depth frames. Model arrival is simulated at 5.5 seconds. Baseline first-confirmation work: 120.79 ms; updated: 30.09 ms. New first depth preview: 20.50 ms, submitted to the renderer in 25.94 ms. Comparable 8 Hz tracking P95: 34.79 → 28.06 ms. Renderer update P95: 5.37 → 5.28 ms. These measure CPU work/submission, not display scanout or network inference.
- A separate sequence at the new 30 Hz timestamp cadence completed 119/119 local tracks, P95 13.97 ms, minimum target-box IoU 0.868. This is a short synthetic motion check, not a measured sustained camera frame rate. The source-reply continuity cache hit, tap and box previews, expiry, uncertainty, selection reset, renderer reattachment after reset and inactive GPU slots all passed. Repeated source JPEG encoding was a cache hit (~0.0005 ms versus 4.16 ms cold). [Full report](capture-speed-device-results.json).
- Live native Astra check with the new shared transport and cache routing key: selection 5.78 s, movement 5.20 s, HTTP 200 and whole-object IoU 1.0 for both. The Mac companion remained off. The first unseen object's semantic confirmation still takes seconds; the immediate depth preview removes the blank wait. Provider prompt-cache hits were not measured or claimed.
- Validation: 83 XCTest + 2 Swift Testing core checks pass; the legacy server suite has 38 tests, 2 opt-in live cases skipped, zero failures. Signed Debug and Release iPhone builds pass. Additional core coverage checks scanline depth sampling against point-in-polygon sampling, cache isolation/expiry, bounded depth previews, changing live images and replacement of reference crops. The final signed Release build is installed and launched on the iPhone with its normal camera entry point. Real-world fast motion, long-session thermal performance and small/occluded-object accuracy still need field testing.
