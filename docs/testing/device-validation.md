...
- Latest installed phone code: `9ecf7f4`. Signed build, installation, and launch pass; 41 XCTest and 2 Swift Testing checks pass. Tight selections use external background evidence with slope correction. Fresh separated depth components can update position after relocation using size, color, and support checks in matching tracking coordinates; propagated old support keeps its stricter depth guard. Independent reviews pass. The new physical move/restore test is pending.
